...
bool ModuleSceneKen::Start()
{
	...
	if (!App->audio->PlayMusic("..\\Game\\ken.ogg")) printf("coudl not load ken.ogg");
	return true;
}
...
update_status ModuleSceneKen::Update()
{
	...
	if (!shipUp && count < 4) {
		count = count + 1;
		if (count >= 4) shipUp = true;
	}
	if (shipUp && count > 0) {
		count = count - 1;
		if (count <= 0) shipUp = false;
	}
	// Draw everything --------------------------------------
	...
	App->renderer->Blit(graphics, 0, count, &red_ship, 1.0);


	//App->renderer->Blit(graphics,0,0,&red_ship, 1.0);

	// TODO 6: Draw the girl. Make sure it follows the ship movement!
	App->renderer->Blit(graphics, 192, 104+count, &(girl.GetCurrentFrame()), 1.0f); // flag animation
